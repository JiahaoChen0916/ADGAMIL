# dataset_modules/wsi_dataset.py
...
class Wsi_Region(Dataset):
    ...
    args:
        ...
        bot_right: tuple of coordinates representing the bottom right corner of WSI region (Default: None)
        ...
        contour_fn (str): 
            contour checking function to use
            choices: ['four_pt_hard', 'four_pt_easy', 'center', 'basic'] (Default: 'four_pt_hard')
        ...
        attention_only (bool): whether to process only attention-related patches (Default: False)
    '''
    def __init__(
        self, 
        wsi_object, 
        ...
        attention_only=False  # Added parameter
    ):
        self.custom_downsample = custom_downsample
        self.attention_only = attention_only  # Store the parameter

        # Downscale factor relative to level 0
        self.ref_downsample = wsi_object.level_downsamples[level]
        # Patch size relative to level 0
        ...
        if self.custom_downsample > 1:
            ...
            patch_size = tuple(
                (np.array(patch_size) * np.array(self.ref_downsample) * custom_downsample).astype(int)
            )
        ...
        for cont_idx, contour in enumerate(wsi_object.contours_tissue): 
            print('processing {}/{} contours'.format(cont_idx + 1, len(wsi_object.contours_tissue)))
