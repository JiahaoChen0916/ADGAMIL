...
class Engine(object):
    ...
    def train(self, data_loader, model, criterion, optimizer):
        ...
        for batch_idx, (data_ID, data_WSI, data_Event, data_Censorship, data_Label) in enumerate(dataloader):
            ...
            # prediction
            logits_ins, logits_bag = model(data_WSI)
            hazards_ins = torch.sigmoid(logits_ins)
            S_ins = torch.cumprod(1 - hazards_ins, dim=1)
            hazards_bag = torch.sigmoid(logits_bag)
            S_bag = torch.cumprod(1 - hazards_bag, dim=1)
            loss = criterion(hazards=hazards_ins, S=S_ins, Y=data_Label.repeat(hazards_ins.shape[0]), c=data_Censorship.repeat(hazards_ins.shape[0]))
            loss = loss + criterion(hazards=hazards_bag, S=S_bag, Y=data_Label, c=data_Censorship)
            # results
            risk = -torch.sum(S_bag, dim=1).detach().cpu().numpy()
