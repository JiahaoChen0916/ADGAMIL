...
class Engine(object):
    ...
    def train(self, data_loader, model, criterion, optimizer):
        ...
        for batch_idx, (data_ID, data_WSI, data_Event, data_Censorship, data_Label) in enumerate(dataloader):
            ...
            # prediction
            logits, loss_aux = model(data_WSI, i=self.epoch * len(dataloader) + batch_idx)
            hazards = torch.sigmoid(logits)
            S = torch.cumprod(1 - hazards, dim=1)
            loss = criterion(hazards=hazards, S=S, Y=data_Label, c=data_Censorship) + 0.5 * loss_aux
            # results
            ...
            optimizer.step()
            optimizer.zero_grad()
            model.update_teacher(self.epoch * len(dataloader) + batch_idx)
