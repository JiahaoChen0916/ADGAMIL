...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
        ...
        # Robust handling for slides where no valid contour hierarchy is returned
        ...
        # Additional safeguard against malformed hierarchy
        if hierarchy.ndim != 2 or hierarchy.shape[1] < 4:
            self.contours_tissue = []
            self.holes_tissue = []
            return
        ...
        # If everything is filtered out, keep empty lists instead of crashing later
        if foreground_contours is None or len(foreground_contours) == 0:
            self.contours_tissue = []
            self.holes_tissue = []
            return
